...
export default function UserChatBubble() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {isOpen && (
        <div
          className={`w-[420px] bg-card border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-200 ${
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-modal flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-text-primary text-sm font-semibold truncate max-w-[140px]">
                {headerTitle}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1.5 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-raised transition-colors text-xs leading-none"
              >
                —
              </button>
              <button
                ...
                <>
                  <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-2.5">
                    {activeMessages.length === 0 ? (
                      <div className="flex items-center justify-center h-full text-text-muted text-xs select-none">
                    ...
                    {isActiveTyping && (
                      <div className="flex justify-start">
                        <div className="bg-overlay border border-border rounded-2xl rounded-bl-sm px-3.5 py-2 flex gap-1 items-center">
      ...
      <button
        onClick={toggleChat}
        className="w-14 h-14 rounded-full bg-accent hover:bg-accent/90 text-white shadow-xl shadow-blue-900/40 flex items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95"
      >
