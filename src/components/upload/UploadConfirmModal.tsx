          ...
          {pending.map((p, idx) => (
            <div key={idx} className="flex items-center gap-3 bg-[#161925] rounded-xl px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-slate-200 text-xs font-medium truncate">{p.file.name}</p>
                <p className="text-slate-500 text-[10px]">{formatBytes(p.file.size)}</p>
              </div>
              <button
                onClick={() => onRemove(idx)}
                className="p-1 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-400/10 transition-colors flex-shrink-0"
              >
                <X size={13} />
              </button>
            </div>
