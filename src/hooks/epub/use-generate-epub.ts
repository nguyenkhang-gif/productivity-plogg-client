...
interface Options {
  onSuccess?: (data: string) => void; // Hàm được gọi khi đăng nhập thành công
...
export const useGenerateEpub = () => {
  ...
  const generateEpub = async (epub: epubOptionsI, options?: Options) => {
    try {
      setData(null);
      setError(null);
      setStatus("pending");
      ...
      const res = await axiosInstance.post("/epub/generate-epub", { data: epub });
